...
rra_interactions <- function(
    ...
    sample_id = NULL, output_dir = ".", n_perm = 1000) {
    #  Sanity checks
    ...
    obj_cellchat <- readRDS(cellchat_obj) %>% tidyr::unite(unique_combi, source_target, complex_interaction, sep = "|", remove = FALSE)

    obj_liana <- readRDS(liana_obj) %>% tidyr::unite(unique_combi, source_target, complex_interaction, sep = "|", remove = FALSE)

    obj_cell2cell <- readRDS(cell2cell_obj) %>% tidyr::unite(unique_combi, source_target, complex_interaction, sep = "|", remove = FALSE)

    obj_cpdb <- readRDS(cpdb_obj) %>% tidyr::unite(unique_combi, source_target, complex_interaction, sep = "|", remove = FALSE)
    ...
    connectome <- obj_liana %>%
        dplyr::select(unique_combi, source_target, complex_interaction, Sample, connectome.rank) %>%
        dplyr::mutate(method = "Connectome (LIANA)")

    logfc <- obj_liana %>%
        dplyr::select(unique_combi, source_target, complex_interaction, Sample, logfc.rank) %>%
        dplyr::mutate(method = "LogFC (LIANA)")


    natmi <- obj_liana %>%
        dplyr::select(unique_combi, source_target, complex_interaction, Sample, natmi.rank) %>%
        dplyr::mutate(method = "NATMI (LIANA)")

    sca <- obj_liana %>%
        dplyr::select(unique_combi, source_target, complex_interaction, Sample, sca.rank) %>%
        dplyr::mutate(method = "SCA (LIANA)")

    cytotalk <- obj_liana %>%
        dplyr::select(unique_combi, source_target, complex_interaction, Sample, cytotalk.rank) %>%
    ...
    obj_cellchat <- obj_cellchat %>%
        ...
        dplyr::mutate(CellChat.rank = dplyr::dense_rank(dplyr::desc(log10_score)))

    obj_cell2cell <- obj_cell2cell %>%
        dplyr::mutate(pval_dummy = ifelse(pval == 0, 1 / n_perm, pval)) %>%
        dplyr::mutate(log10_score = -log10(pval_dummy) * Cell2Cell_score) %>%
        dplyr::mutate(Cell2Cell.rank = dplyr::dense_rank(dplyr::desc(log10_score)))

    obj_cpdb <- obj_cpdb %>%
        dplyr::select(-rank) %>%
        dplyr::mutate(pval_dummy = ifelse(pval == 0, 1 / n_perm, pval)) %>%
        dplyr::mutate(log10_score = -log10(pval_dummy) * CellPhoneDB_score) %>%
        dplyr::mutate(CellPhoneDB.rank = dplyr::dense_rank(dplyr::desc(log10_score)))

    tool_res <- list(connectome, sca, cytotalk, natmi, logfc, obj_cellchat, obj_cell2cell, obj_cpdb)
    ...
    ranked_interactions <- RobustRankAggreg::aggregateRanks(rmat = rank_mat, method = "RRA") %>%
        ...
        dplyr::left_join(obj_cell2cell %>%
            ...
            dplyr::select(Sample, source_target, complex_interaction, cell2cell, Cell2Cell_score)) %>%
