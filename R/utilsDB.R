...
#' @export
CheckLengthGeneSymbol <- function(vec) {
...
#' @export
MakeComplex <- function(vec, collapse = collapse) {
...
#' @param collapse character string to separate items default = ':'
#' @export
MakeComplex_wrapper <- function(x, collapse = ":") {
    MakeComplex(OrderVector(x), collapse = collapse)
}
...
#' @export
#' @importFrom dplyr %>%
LookupUniprot <- function(gene, gene_uniprot_table) {
    gene_uniprot_table %>%
...
IsMapped <- function(vec, lookup_table, lookup_var = "genesymbol") {
...
AddProteins <- function(vec, gene_uniprot_table) {
    tmp <- paste0(
        lapply(
            vec[!is.na(vec)],
            LookupUniprot,
...
#' @export
#' @importFrom dplyr %>%
GetProteinsWrapper <- function(genes_df) {
