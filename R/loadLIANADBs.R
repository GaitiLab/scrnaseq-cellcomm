...
#' @export
#' @importFrom dplyr %>%
LoadLIANADBs <- function() {
    # Loading Consensus database
    liana_consensus <- liana::select_resource("Consensus")[[1]] %>%
        dplyr::mutate(method = "LIANA consensus")

    # load Ramilowski 2015 database
    liana_ramilowski <- liana::select_resource("Ramilowski2015")[[1]] %>%
        dplyr::mutate(method = "LIANA Ramilowski2015")

    # Account for missing columns
    missing_liana <- setdiff(
        colnames(liana_consensus),
        colnames(liana_ramilowski)
    )
    missing_ramilowski <- setdiff(
        colnames(liana_ramilowski),
        colnames(liana_consensus)
    )
