...
take_consensus <- function(
    ...
    all_votes <- interactions_signif %>%
        ...
        dplyr::left_join(obj_liana %>% dplyr::select(dplyr::all_of(cols_oi), LIANA_score) %>% dplyr::mutate(in_liana = 1)) %>%
        dplyr::left_join(obj_cellchat %>% dplyr::select(dplyr::all_of(cols_oi), CellChat_score) %>% dplyr::mutate(in_cellchat = 1)) %>%
        dplyr::left_join(obj_cell2cell %>% dplyr::select(dplyr::all_of(cols_oi)) %>% dplyr::mutate(in_cell2cell = 1)) %>%
        dplyr::left_join(obj_cpdb %>% dplyr::select(dplyr::all_of(cols_oi), CellPhoneDB_score) %>% dplyr::mutate(in_cpdb = 1))
