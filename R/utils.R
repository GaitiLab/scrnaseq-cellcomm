...
get_metadata <- function(input_file, output_dir = ".") {
    ...
    saveRDS(
        ...
        glue::glue("{output_dir}/{out_filename}__metadata.rds")
    )
    message("Save metadata as csv file...")
    write.csv(
        seurat_obj@meta.data,
        glue::glue("{output_dir}/{out_filename}__metadata.csv")
    )
    message("Finished...")
}
...
reduce_seurat_obj <- function(input_file, output_dir = ".") {
    ...
    saveRDS(
        seurat_obj,
        glue::glue("{output_dir}/{out_filename}_reduced_size.rds")
    )
    message("Finished...")
}
...
#' @export
split_seurat_into_samples <- function(
    input_file,
    output_dir = ".",
    sample_var = "Sample"
) {
