...
#' @export
run_liana <- function(
    gene_expr, interactions_db, annot,
    output_dir = ".", min_cells = 5, min_pct = 0.1, n_perm = 1000) {
    #  Sanity checks
    if (!(file.exists(gene_expr) && endsWith(gene_expr, ".rds"))) {
        stop("Seurat object ('gene_expr') does not exists or is not an RDS object")
    }
    if (!(file.exists(interactions_db) && endsWith(interactions_db, ".rds"))) {
        stop("Interactions database ('interactions_db') does not exists or is not an RDS object")
    }
    ...
    # ---- Run LIANA ----
    liana_obj <- liana::liana_wrap(seurat_obj,
        method = methods,
        resource = "custom",
