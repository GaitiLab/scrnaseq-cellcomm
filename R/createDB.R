...
#' @export
CreateDB <- function(source_cpdb_dir, output_dir) {
    ...
    liana_db <- LoadLIANADBs()

    message("(2/12) Retrieving CellChat DB...")
    cellchat_db <- LoadCellChatDB()

    message("(3/12) Retrieving CPDB DB...")
    cpdb_db <- LoadCPDBDB(source_cpdb_dir = source_cpdb_dir)

    message("(4/12) Combining databases...")
    combi_db <- HardCombineDBs(
    ...
    ref_db <- combi_db_filtered$db %>%
        dplyr::select(
            ...
            method,
            is_dupl_undirected,
            complex_interaction_OLD
        ) %>%
        dplyr::mutate(
            ligand_complex = stringr::str_replace_all(
                source_genesymbol,
                "_",
                ":"
            ),
            receptor_complex = stringr::str_replace_all(
                target_genesymbol,
                "_",
                ":"
            ),
    ...
    message(
        "(10/12) Save unified database for Cell2Cell (same format as LIANA)..."
    )
    write.csv(
        combi_db_filtered$db,
        glue::glue("{output_dir}/cell2cell_db.csv"),
        row.names = FALSE
    )
    ...
    python_script <- system.file(
        "Python/update_cellphonedb.py",
        package = "scrnaseq.cellcomm"
    )
    ...
    if (file.exists(glue::glue("{output_dir}/cellphonedb.zip"))) {
        message(
            "Clean-up redundant files used for generating 'cellphonedb.zip'..."
        )
        files_to_remove <- c(
            "complex_input",
            "gene_input",
