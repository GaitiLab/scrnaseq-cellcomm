...
format_cpdb <- function(
    ...
    sample_id = NULL) {
    #  Sanity checks
    if (!(file.exists(interaction_scores) && endsWith(interaction_scores, ".txt") && stringr::str_detect(interaction_scores, "statistical_analysis_interaction_scores"))) {
        stop("'statistical_analysis_interaction_scores' file ('interaction_scores') does not exists or is not a txt file")
    }
    if (!(
        file.exists(pval) && endsWith(pval, ".txt") && stringr::str_detect(pval, "statistical_analysis_pvalues"))) {
        stop("'statistical_analysis_pvalues' file ('pval') does not exists or is not a txt file")
    }
    if (!(file.exists(sign_means) && endsWith(sign_means, ".txt") && stringr::str_detect(sign_means, "statistical_analysis_significant_means"))) {
        stop("'statistical_analysis_significant_means' file ('sign_means') does not exists or is not a txt file")
    }
    ...
    sign_means <- read.table(sign_means, sep = "\t", header = TRUE, check.names = FALSE)
    means <- read.table(means, sep = "\t", header = TRUE, check.names = FALSE)
    interaction_scores <- read.table(interaction_scores, sep = "\t", header = TRUE, check.names = FALSE)
    ...
    pval <- pval %>%
        ...
        dplyr::mutate(source_target = stringr::str_replace_all(source_target, "\\|", "__"))

    message("Process 'sign_means'...")
    colnames_oi <- colnames(sign_means)[stringr::str_detect(colnames(sign_means), "\\|")]
    ...
    colnames_oi <- colnames(interaction_scores)[stringr::str_detect(colnames(interaction_scores), "\\|")]
    ...
    interactions <- pval %>%
        dplyr::left_join(sign_means, by = c("interacting_pair", "source_target")) %>%
    ...
    message("Save output...")
    saveRDS(interactions, glue::glue("{output_dir}/cpdb__{sample_id}__postproc.rds"))
