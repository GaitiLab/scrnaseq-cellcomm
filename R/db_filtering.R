...
db_filtering <- function(db) {
    ...
    db <- db %>%
        dplyr::mutate(
            source_genesymbol_ordered := apply(db %>% select(starts_with("source_genesymbol_subunit")), 1, make_complex_wrapper, "_"),
            target_genesymbol_ordered := apply(db %>% select(starts_with("target_genesymbol_subunit")), 1, make_complex_wrapper, "_")
        ) %>%
