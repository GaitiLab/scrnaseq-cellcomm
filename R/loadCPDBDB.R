...
#' @importFrom dplyr %>%
#' @export
LoadCPDBDB <- function(source_cpdb_dir) {
    cpdb_interaction_input <- data.table::fread(
        ...
        dplyr::mutate(
            ...
            source_genesymbol_complex = dplyr::case_when(
                nchar(unit_2) == 1 ~ paste0(unit_1, "-", unit_2),
                TRUE ~ unit_1
            ),
            target_genesymbol_complex = dplyr::case_when(
                nchar(unit_2) == 1 ~ unit_3,
                nchar(unit_3) == 1 ~ paste0(unit_2, "-", unit_3),
                TRUE ~ unit_2
            )
        ) %>%
        # Format source_genesymbol and target_genesymbol
        dplyr::mutate(
            source_genesymbol = stringr::str_replace_all(
                source_genesymbol_complex,
                "\\+",
                "_"
            ),
            target_genesymbol = stringr::str_replace_all(
